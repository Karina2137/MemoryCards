var cards = ['eren.png', 'mikasa.png', 'armin.png', 'levi.png', 'hange.png', 'erwin.png','eren.png', 'mikasa.png', 'armin.png', 'levi.png', 'hange.png', 'erwin.png'];
...
function revealCard(nr){
    ...
    if (opacityValue != 0 && lock==false){
        ...
        if (oneVisible==false){
            ...
            $('#c'+nr).addClass('cardA');
            $('#c'+nr).removeClass('card');
            if(cards[visible_nr] == cards[nr] && nr !=visible_nr) {
                setTimeout(function() {hide2Cards(visible_nr, nr)}, 750);
                turnCounter++;
                $('.value').html(turnCounter);
                oneVisible=false;
            }else if(nr==visible_nr){
                lock=false;
            }else{
                setTimeout(function() {restore2Cards(visible_nr, nr)}, 1000);
                turnCounter++;
                $('.value').html(turnCounter);
                oneVisible=false;
            }
